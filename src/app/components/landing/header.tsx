      ...
      >
        ...
        <nav aria-label="Primary navigation" className="hidden md:flex items-center gap-7 text-sm font-medium">
          {navLinks.map(({ href, label }) => (
            ...
            >
              {label}
              <span className="absolute -bottom-0.5 left-0 h-px w-0 bg-emerald-500 transition-all duration-200 group-hover:w-full" />
            </Link>
          ))}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold px-5 py-2 transition-colors duration-150 shadow-md shadow-emerald-900/40"
          >
            Register Now
        ...
        <div className="flex items-center gap-6 text-xs font-medium text-slate-500 animate-marquee whitespace-nowrap">
          {tickerWords.map((word, i) => (
            <span key={i} className="flex items-center gap-3">
              <span>{word}</span>
              <svg className="w-3 h-3 text-emerald-500/60 flex-shrink-0" viewBox="0 0 12 12" fill="currentColor">
      ...
      {open && (
        ...
        >
          ...
          <Link
            ...
            rel="noopener noreferrer"
            onClick={() => setOpen(false)}
            className="mt-2 inline-flex items-center gap-2 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold px-6 py-2.5 transition-colors"
          >
            Register Now →
          </Link>
        </nav>
